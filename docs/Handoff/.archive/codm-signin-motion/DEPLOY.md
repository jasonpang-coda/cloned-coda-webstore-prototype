...
2. Set **Root Directory** to `docs/Handoff/codm-signin-motion` (Edit → enter path → Continue).
...
4. Deploy. Copy the production URL (e.g. `codm-signin-motion-handoff.vercel.app`).
...
### Motion tokens on Vercel

Vercel cannot read files outside the Root Directory during build. Token CSS lives in [`vendor/tokens/`](./vendor/tokens/) (vendored copies of `src/tokens/*.css`). After changing prototype motion tokens locally, run `npm run sync-tokens` and commit the updated vendor files.
