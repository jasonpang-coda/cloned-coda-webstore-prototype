...
<div style="display:grid;gap:12px;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));margin:16px 0;">
...
#### `--motion-sku-enter`
- **Value:** `250ms var(--motion-ease-decelerate)`
...
animation-duration: var(--motion-duration-slow);     /* 350ms */
animation-timing-function: var(--motion-ease-decelerate);
animation-fill-mode: both;
```

#### `--motion-sku-exit`
- **Value:** `200ms var(--motion-ease-accelerate)`
...
#### `--motion-modal-enter`
- **Value:** `350ms var(--motion-ease-decelerate)` (from base `motion.css`)
...
#### `--motion-modal-exit`
- **Value:** `200ms var(--motion-ease-accelerate)` (from base `motion.css`)
...
- **Value:** `350ms var(--motion-ease-spring)` — `cubic-bezier(0.34, 1.56, 0.64, 1)`
...
- **Value:** `250ms var(--motion-ease-accelerate)`
...
- **Direction:** rises from 24px below rest (`--motion-distance-xl`) — conveys "this content was below" without a full off-screen slide. Not edge-anchored (no persistent off-screen position), so a full `translateY(100%)` would read as spatially incorrect.
...
#### `--motion-sku-hover-in`
- **Value:** `220ms var(--motion-ease-decelerate)`
...
#### `--motion-sku-hover`
- **Value:** `150ms var(--motion-ease-standard)`
...
#### `--motion-sku-press`
- **Value:** `100ms var(--motion-ease-standard)`
...
#### `--motion-ripple`
- **Value:** `var(--motion-duration-slow)` — **350ms**
...
.fx-ripple .fx-ripple__wave {
  animation: ripple var(--motion-ripple) var(--motion-ease-decelerate) forwards;
}
...
#### `--motion-sku-select`
- **Value:** `250ms var(--motion-ease-standard)`
...
#### `--motion-sku-badge`
- **Value:** `350ms var(--motion-ease-spring)`
...
- **Value:** `350ms var(--motion-ease-decelerate)`
...
#### `--motion-sku-stagger`
- **Value:** `var(--motion-stagger-sm)` — **90ms per card** (overridden from library default 50ms)
...
- **Easing:** paired with `--motion-ease-standard`
...
- **Animation:** `@keyframes spin`, paired with `--motion-ease-linear` (continuous loop)
...
The FCM category content area swaps between categories with a fade-in/fade-out crossfade wrapped in a `cat-catalog` Vue Transition (`mode="out-in"`). Each incoming category fades up from `--motion-distance-md` (8px); the outgoing category fades out flat.

```css
.cat-catalog-enter-active {
  animation-name: cat-fade-in;
  animation-duration: var(--motion-duration-base);        /* 250ms */
  animation-timing-function: var(--motion-ease-decelerate);
  animation-fill-mode: both;
}
.cat-catalog-leave-active {
  animation-name: cat-fade-out;
  animation-duration: var(--motion-duration-exit);        /* 200ms */
  animation-timing-function: var(--motion-ease-accelerate);
  animation-fill-mode: both;
}
...
.sku-image-card:hover {
  ...
  transform: translateY(calc(-1 * var(--motion-distance-sm)));  /* -4px */
}
/* press */
...
**Two-stage navigate sequence:** when a nav item with an anchor is tapped, `emit('close')` fires immediately so the drawer begins its 200ms exit. Navigation (scroll for COD:M, category filter switch for FCM) is deferred until after the exit completes — the delay is read live from `--motion-duration-exit` via `getComputedStyle` so it stays in sync with the token without a hardcoded magic number.

```js
// NavDrawer.vue — onItemClick
emit('close')
const ms = parseInt(
  getComputedStyle(document.documentElement).getPropertyValue('--motion-duration-exit'), 10
) || 200
...
```css
--motion-nav-enter: var(--motion-duration-slow) var(--motion-ease-decelerate); /* 350ms ease-out */
...
- Enter: `--motion-duration-base` (250ms, decelerate) — slightly shorter than the bottom nav (no cascade delay needed, triggered by scroll)
- Exit: `--motion-duration-exit` (200ms, accelerate) — decisive departure when scrolling back up
...
@keyframes slide-down {
  from { opacity: 0; transform: translateY(calc(-1 * var(--motion-distance-lg))); }
  to   { opacity: 1; transform: translateY(0); }
}
```
