...
source: src/library/stories/Card.stories.js
variants: 2
states: 1
---
...
> Plain bordered L2 content pane — border/background match BaseSheet's own container tier, but static (no scrim, no transition, no header/footer slots). Grounded in OrderCompletePage's Order Summary / Need Help panes, the first two identical repeats of this exact wrapper.

**Generated Component Spec from Story Harness.** Generated deterministically from the component's story declaration. Regenerate with `node scripts/export-handoff.mjs --story card`.
