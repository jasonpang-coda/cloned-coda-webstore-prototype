...
| `src/components/DeviceToolbar.vue` | Segmented switcher (`v-model:device`) + store dropdown (`useTheme`) + DEMO body-font dropdown (`useBodyFont`, COD:M only) + version label (right-aligned, reads `version` from `package.json`). **(v0.29.0)** Library toggle button (`view_module` icon, `useLibrary().toggle()`) added to the dev-tools segment — hidden in store-locked builds. **(v0.35.0)** Collapsible: a sticky icon-only tab handle (bottom of the bar) expands/collapses the segments via `grid-template-rows` (`--motion-accordion`) + chevron rotate/opacity (`--motion-toggle`); state lives in `useToolbarCollapse` (singleton) so the `/` command console can toggle it too. On touch devices (`useDeviceDetect`) the bar starts collapsed and the device-frame segment is hidden. The component measures its own height live via `ResizeObserver` and writes `--toolbar-h`. |
...
| `src/composables/useToolbarCollapse.js` | **(v0.35.0)** Singleton `collapsed` ref + `collapse()`/`expand()`/`toggle()`/`set()` — mirrors `useInspector`/`useLibrary`. Shared between `DeviceToolbar` and `CommandConsole` (`/` → "Collapse/Expand preview toolbar"). |
...
Region + language pickers (Figma 4014:4802 / 4015:5635 / 4016:6248). Both read `useLocale` (singleton, `globalThis`-pinned like `useTheme`): `regionSelectorOpen` / `languageSelectorOpen`, `setRegion()` / `setLanguage()`. CheckoutSheet chrome: scrim (no blur) + frosted `--bg-sheet` panel, z-4, bottom sheet on mobile → centered modal at `@media (min-width: 801px)` when `!isMobile` (`position: fixed`, scale+fade entrance). Mounted in the `#overlay` slot from `App.vue` with `:is-mobile="device !== 'none'"`.
...
**Responsive (`:is-mobile`):** framed = bottom sheet (`position:absolute`, slide-up). Responsive (`.sheet--responsive`) = `position:fixed`; at M+ becomes a centered modal (`max-width:420px`, `max-height:80vh`, fade+scale entrance). Z-index 4.

**v0.38.0 bottom scroll scrim + 80vh modal cap (universal — every sheet):** Applies identically to `CheckoutSheet`, `ItemSummarySheet`, `ClaimGiftSheet`, `SignInSheet`, `RegionSelectorSheet`, `LanguageSelectorSheet`. Two changes:
- **Responsive modal `max-height` capped at `80vh`** (was `85vh`) across all six — leaves more surrounding chrome visible on short desktop viewports.
- **Bottom scroll scrim** — the scrolling body is wrapped in a **non-scrolling `__body-wrap`** (`position: relative; flex: 1; min-height: 0`) that sits in the panel's flex column where the body used to; the body (`flex: 1; overflow-y: auto`) scrolls inside it. The `__scroll-fade` is a **`position: absolute; left/right/bottom: 0; height: var(--size-img-xl)`** (64px — bumped from `--size-img-l`/48px for visibility) child of that wrapper, so it pins to the scroll viewport's bottom edge and never scrolls with content. (First attempts used `position: sticky` and then a negative-margin flex sibling — both proved unreliable, especially over `RegionSelectorSheet`'s CSS-multi-column list; the relative-wrapper + absolute-child pattern is the robust fix.) Fill is the new `--gradient-scroll-fade-bottom` token (extensions.css — `linear-gradient(to top, var(--scrim-banner), transparent)`, built from the neutral scrim ramp not `--bg-page`, so it reads correctly over a themed gradient sheet fill). Visibility is driven by `useBottomFade(bodyRef)` (`src/composables/useBottomFade.js`): a small, non-singleton composable — `watch(elRef, …)` so it self-(re)binds whenever the body element mounts/unmounts under the sheet's own `v-if`, tracking `scrollTop + clientHeight < scrollHeight - 1` via `ResizeObserver` + a passive `scroll` listener. Shows only while the body actually has more content below the fold; hides once scrolled to the end or if content never overflows. `SignInSheet`'s body previously had no scroll mechanism at all (`.signin-sheet__body` lacked `overflow-y: auto`) — added as part of this change so it can genuinely scroll rather than clip.

**v0.38.0 search-box scrim + gap (`RegionSelectorSheet` only):** `.selector__search` gained `margin-bottom: var(--gap-content-default)` (8px) — a visible gap before the market list, so the search box's own outer edge doesn't butt against the list. A second, always-visible `.selector__search-fade` sits at `top: 0` of `.selector__body-wrap` (i.e. right after that gap, not overlapping the search box) — half the bottom scrim's height (`calc(var(--size-img-xl) / 2)`), reusing the exact same `--gradient-scroll-fade-bottom` background but with `transform: scaleY(-1)` so it darkens at its top edge (flush with the gap) and fades down into the list, the mirror image of the bottom scrim's orientation. Unlike the bottom scrim it isn't gated on `useBottomFade` — the search box stays in view regardless of scroll position, so the separator is permanent.
