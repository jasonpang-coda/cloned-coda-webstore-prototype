...
|---|---|---|
| `success` | `[10, 40, 20]` | Global success pattern — light pulse, gap, medium settle. Fires on: sign-in success snackbar (`useAuth`), gift claim confirmed (`useGiftClaim.confirmClaim()` + EA auto-confirm), checkout CTA tap (`CheckoutSheet`). |
...
| BUY NOW tap (press) | `35` |
| BUY NOW tap (click — after press) | `[10, 40, 20]` |
| Gift claim confirmed | `[10, 40, 20]` |
| EA auto-confirm (post sign-in) | `[10, 40, 20]` |
...
- **FE rules of engagement:** `.claude/skills/web-store-fe/`
