...
> ⚠️ **Token names in this document predate the `--x-` prefix migration.**
> Every token below is written as `--motion-*`; the real token is
> `--x-motion-*` (e.g. `--motion-sku-enter` → `--x-motion-sku-enter`). Copying
> a name from here as-is resolves to nothing. For current token names, the
> component/token map, and the material-driven motion doctrine, see the
> `.claude/skills/web-store-motion/` skill — that is the authoritative source
> for names. This document is still useful for its choreography detail
> (per-interaction timing/sequencing), just re-verify any token name against
> `src/tokens/` before using it.
...
#### `--motion-sku-enter`
- **Value:** `350ms var(--motion-sys-ease-decelerate)` (aliases `--motion-sys-duration-slow`)
...
#### `--motion-sku-shimmer`
- **Value:** `3000ms` (aliases `--motion-shimmer-sweep`)
...
- **Animation:** `@keyframes shimmer-loop`, `--motion-sys-ease-linear`
- **Color:** `--gradient-bestseller-metallic-shine` (white metallic sheen), opacity via `--fx-bestseller-shimmer-opacity` (`0.25`)
...
- Compact best-seller carousel cards (image `::after`, white metallic, `--motion-sku-shimmer` 3s)
...
**Material variants (v0.65.0):** the flat `--fx-shimmer-color` override shown above is the pre-v0.65.0 API — still supported on the bare `.fx-shimmer` class, but every primary CTA (`BuyNowBar`, `ClaimGiftSheet`, the Payment/Checkout/Info step footers) now pairs `.fx-shimmer` with a material modifier instead:

```html
<button class="sheet__checkout fx-shimmer fx-shimmer--metal-gloss">
```

- `.fx-shimmer--metal-gloss` / `--glass` / `--paper-coated` override only the `::after` background, reading their shape from `src/tokens/materials.css`, which sources its angle/colour from `src/tokens/light.css`'s `--x-light-gradient-angle`/`--x-light-color` instead of a hardcoded `100deg`/flat white. `metal-gloss` is asymmetric (long dim trailing tail, sharp leading edge) and hue-shifts from `--x-material-metal-gloss-shimmer-core` (set per-button to that button's own fill token) at the core to `-rim` (the light colour) at the edges — this is what makes each CTA's sweep tint toward its own brand colour instead of a generic white streak.
- No variant exists for matte metal or uncoated paper — see `.fx-ambient-swell` below.
- Full physics rationale: `.claude/skills/material-fx` (`fx-catalogue.md`'s Sheen-sweep entry, `materials.css`'s own comments).

### `.fx-ambient-swell` (v0.65.0)
**Defined in:** `src/tokens/effects.css`

The matte-metal counterpart to `.fx-shimmer` — a full-surface tint whose *opacity* breathes (reuses the `pulse` keyframe), not a travelling sweep, since matte metal has no discrete highlight to animate. `.fx-ambient-swell--metal-matte` reads `--x-material-metal-matte-ambient-color`. No consumer in this repo yet uses it — it exists so a future matte-metal surface has the physically-correct effect available instead of reaching for `.fx-shimmer`.
...
## Transaction History (v0.33.0)

The Transaction History feature reuses **only existing tokens** — no new motion tokens were added.

### `dropdown` (Vue Transition name) — `FilterDropdown.vue`
Drives the options panel open/close and the trigger chevron rotation.

```css
/* Panel enter */
.dropdown-enter-active {
  transition: opacity var(--motion-dropdown), transform var(--motion-dropdown);
}
/* Panel exit */
.dropdown-leave-active {
  transition:
    opacity    var(--motion-sys-duration-exit) var(--motion-sys-ease-accelerate),
    transform  var(--motion-sys-duration-exit) var(--motion-sys-ease-accelerate);
}
.dropdown-enter-from,
.dropdown-leave-to {
  opacity: 0;
  transform: translateY(calc(-1 * var(--motion-sys-distance-sm))) scale(0.98);
}
/* Chevron */
.filter-dropdown__chevron { transition: transform var(--motion-dropdown); }
.filter-dropdown__trigger.is-open .filter-dropdown__chevron { transform: rotate(180deg); }
```

- **`--motion-dropdown`** = `200ms var(--motion-sys-ease-decelerate)` — enter eases out (arrives fast, settles gently); exit uses accelerate to fly off cleanly.
- `transform-origin: top` — panel grows from the trigger edge, not the viewport center.
- Scale is `0.98→1` — enough to hint at origin without distraction.

### Option row stagger — `FilterDropdown.vue`

```css
.filter-dropdown__option {
  animation-name: dropdown-option-enter;
  animation-duration: var(--motion-sys-duration-base);
  animation-timing-function: var(--motion-sys-ease-decelerate);
  animation-fill-mode: both;
  animation-delay: var(--opt-delay, 0ms); /* inline: calc(i * --motion-sys-stagger-sm) */
}
```

Each row fades + rises 4px (same translate axis as the panel), stepped by `--motion-sys-stagger-sm` (50ms). Utilitarian — no spring.

### Card entrance stagger — `TransactionCard.vue` + `TransactionHistoryPage.vue`
...
.txn-card {
  ...
  animation-timing-function: var(--motion-sys-ease-decelerate);
  animation-fill-mode: both;
  /* animation-delay set via inline style (animDelay prop) */
}
...
Cards stagger by 80ms per index (matches `--motion-sys-stagger-md`), set in `TransactionHistoryPage` as `:anim-delay="i * 80"`. All longhands — the easing token contains commas, so the shorthand is invalid.

---

### Sheet sticky-footer + scrim/panel layering — `BaseSheet.vue` (every sheet), `PaymentStepBody.vue`'s promo T&Cs modal

Every `BaseSheet`-based sheet layers its motion instead of moving as one rigid block:

- **Panel vs. scrim:** the panel settles a beat after the scrim on enter, and the scrim trails a beat behind the panel on exit.
- **Footer:** trails the panel by the same beat, on both enter and exit — same transform/opacity as the panel, just delayed.

```css
--motion-sys-duration-footer-delay: 100ms;    /* footer's lag behind the panel, both directions */
--motion-sys-duration-modal-panel-delay: 60ms; /* panel↔scrim offset — also reused by the promo T&Cs modal below */
```

```css
.sheet-enter-active .sheet__panel {
  transition: transform var(--motion-modal-enter);
  transition-delay: var(--motion-sys-duration-modal-panel-delay);
}
...
.sheet-enter-active .sheet__footer {
  ...
  transition-delay: var(--motion-sys-duration-footer-delay);
}
.sheet-leave-active .sheet__footer {
  transition: transform var(--motion-modal-exit), opacity var(--motion-modal-exit);
  transition-delay: var(--motion-sys-duration-footer-delay);
}
```

The footer now reuses the panel's own `--motion-modal-enter`/`-exit` duration+easing directly (the dedicated `--motion-sheet-footer-enter`/`-exit` tokens from v0.60.7 are retired) — only the delay is footer-specific.

The same `--motion-sys-duration-modal-panel-delay` also staggers the promo-code T&Cs modal (`PaymentStepBody.vue`): its panel settles 60ms after its own scrim on enter, and its scrim trails 60ms behind its panel on exit.

`reduced-motion.css` zeroes every `transition-delay` here too (not just `transition-duration`), so all of this collapses to instant rather than snapping after a lingering delay.
...
**Last updated:** August 2026 · v0.65.0 — `.fx-shimmer` gained material variants (`--metal-gloss`/`--glass`/`--paper-coated`, reading shape/colour from the new `src/tokens/light.css` + `src/tokens/materials.css`) and a non-travelling `.fx-ambient-swell`/`--metal-matte` sibling — see the `.fx-shimmer` section above for the API. Otherwise unchanged since v0.64.0's every `--motion-*` token in this doc renamed with an `--x-` prefix (e.g. `--motion-sys-duration-fast` → `--x-motion-sys-duration-fast`) as part of the prod token-structure reconciliation (`docs/token-reconcile.md`); token names below the previous entries are pre-rename — a follow-up pass should update every reference.
**Version:** v0.63.0 — `BaseSheet`'s sticky-footer layering reworked: footer now reuses the panel's own `--motion-modal-enter`/`-exit` duration+easing directly (the dedicated `--motion-sheet-footer-enter`/`-exit` tokens from v0.60.7 are retired) with just a 100ms delay, on both enter and exit. New `--motion-sys-duration-modal-panel-delay` (60ms) staggers panel↔scrim on the sheet's own open/close AND the promo-code T&Cs modal (`PaymentStepBody.vue`).
**Version:** v0.60.7 — sheet sticky-footer layering documented (`--motion-sheet-footer-enter`/`-exit`/`-delay`; footer settles behind the panel on enter, detaches ahead of it on exit); `reduced-motion.css` now also zeroes `transition-delay`.
**Version:** v0.33.0 — Transaction History motion documented (`dropdown` transition, option-row stagger, card-entrance stagger; all reuse existing tokens)
**Version:** v0.24.0 — all primitive tokens renamed to `--motion-sys-*` namespace
